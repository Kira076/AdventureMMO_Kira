...
public class ItemUtils {
	...
	public static ItemStack build(@Nonnull final ItemType type, final int amount, final int unsafe) {
		return ItemStack.builder().fromContainer(DataContainer.createNew().set(DataQuery.of("ItemType"), type).set(DataQuery.of("Count"), amount).set(DataQuery.of("UnsafeDamage"), unsafe)).build();
	}
	...
	public static ItemStack getPlayerHead(@Nonnull final Player p) {
		return ItemStack.builder().itemType(ItemTypes.SKULL).add(Keys.SKULL_TYPE, SkullTypes.PLAYER).add(Keys.REPRESENTED_PLAYER, p.getProfile()).build();
	}
